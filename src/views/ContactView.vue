...
const topic = computed(() => findCapability(route.query.topic))
const contextKey = computed(() => `${audience.value?.id || ''}:${topic.value?.id || ''}`)
const initialMessage = computed(() => [audience.value ? `I'm interested in the guide for ${audience.value.short.toLowerCase()}.` : '', topic.value ? `I'd like to discuss ${topic.value.label.toLowerCase()}.` : ''].filter(Boolean).join('\n'))
</script>
